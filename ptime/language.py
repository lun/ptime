...
class Language(object):
    ...
    def __init__(self, month_abbreviations, relative_dates, day_offsets):
        ...
        self.day_offsets = dict([(key, set(value)) for key, value in day_offsets.iteritems()])

    def get_month(self, name):
        abbreviation = name[:3]
        if abbreviation in self.month_abbreviations:
            return self.month_abbreviations[abbreviation]
        return None

    def get_offset_for_relative_date(self, relative_date):
        if relative_date in self.relative_dates:
            return self.relative_dates[relative_date]
        return None
